...
export const getProjectServices = (
  ...
  const stateService = getAutoGeneratedAttributes(SERVICE_TYPE.STATE, project, environment)

  if (!stateService) {
    throw new Error('We weren’t able to determine the state backend for the project!')
  }

  const services: ServiceConfiguration[] = [stateService]
  ...
  for (const serviceConfig of serviceConfigurations) {
    ...
    Object.values(service.associations).forEach((association) => {
      const { with: requiredServiceType, where: associationLookup } = association
