...
import type { LocalStateResources } from '@providers/local/services/state'
import type { ProjectConfiguration } from '@core/project'
import type { LocalProviderResources } from '@providers/local/services/provider'
import { PROVIDER, SERVICE_TYPE } from '@constants'
import type { BaseProvisionable, Provisions, ServiceTypeChoice } from '@core/service'
import type { AwsDatabaseDeployableResources } from '@providers/aws/services/database'
...
import { deployment, destruction, getOperationByName, OPERATION_TYPE, setup } from '@core/operation'
...
describe('Operation', () => {
  ...
  const getProvisions = (
    provs: BaseProvisionable[],
    lookup: ServiceTypeChoice | ((p: BaseProvisionable) => boolean),
  ): Provisions => {
    const finder =
      typeof lookup === 'function' ? lookup : (p: BaseProvisionable) => p.service.type === lookup
  ...
  describe('deployment operation', () => {
    ...
    it('populates the provisionables', () => {
      ...
      expect(new Set(provisionables.map((p) => p.service.type))).toEqual(
        new Set([
    ...
    it('registers the AWS provider resources', () => {
      ...
      expect(new Set(Object.keys(provisions))).toEqual(
        new Set(['provider', 'account', 'gateway', 'kmsKey', 'subnets', 'vpc', 'outputs']),
      )

      const { provider, gateway, kmsKey, subnets, vpc, account } =
